...
import logging
import datetime
from lna_module_level_test_manager import LNAModuleTestManager
...
class OscarLNAModuleGUI:
    def __init__(self, root, sim_mode=False):
        self.root = root
        self.root.title("Oscar LNA Module GUI")
        ...
        self.measurement_dropdown = ttk.Combobox(self.extra_tab, textvariable=self.measurement_var, values=["S22", "S21", "S11", "IP3", "Noise_Figure"], state="readonly")
        ...
        if sim_mode:
            ...
            self.lna_module_test_manager = LNAModuleTestManager(sim=False)
    ...
    def run_tests(self):
        ...
        message = f"[{timestamp}] Running tests... SN: {serial_number}, Selected: {selected_option}, Checkbox 1: {checkbox1_state}, Checkbox 2: {checkbox2_state}, CHeckbox 3: {checkbox3_state}"
        log_message(message)
        print(message)
        ...
        if self.sim_mode:
            ...
            if checkbox1_state:
                ...
                options["IP3"] = True
                options["S11"] = True
                options["Gain_Phase"] = True
            else:
                options["S22"] = False
                options["IP3"] = False
                options["S11"] = False
                options["Gain_Phase"] = False
            ...
            if checkbox3_state:
                options["Noise_Figure"] = True
            else:
                options["Noise_Figure"] = False

            # THis is for the results path creation i wanted to see it first to make sure it was working
            self.lna_module_test_manager.na_test.config.new_sno(serial_number, user_defined_dir)
            self.lna_module_test_manager.run_and_process_tests(selected_option, sno=serial_number, options=options)
    ...
    def run_tools(self):
        ...
        timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        message = f"[{timestamp}] Running tools... Selected: {selected_option}, Measurement: {measurement_type}"
        log_message(message)
        print(message)
        self.lna_module_test_manager.run_state_process(selected_option, measurement_type)
        # This reenables it
...
if __name__ == "__main__":
    ...
    try:
        root = tk.Tk()
        app = OscarLNAModuleGUI(root)
        root.mainloop()
    except Exception as e:
        app.lna_module_test_manager.disconnect_instruments()
        root.destroy()
        sys.exit()
        print(e)
