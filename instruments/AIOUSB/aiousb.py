...
def adc_callback(
    pBuf: POINTER(c_uint16), BufSize: c_uint32, Flags: c_uint32, Context: c_uint32
) -> c_uint32:
...
def QueryDeviceInfo(index):
    ...
    status = AIOUSB.QueryDeviceInfo(
        index, byref(PID), byref(len), byref(name), byref(dio), byref(ctrs)
    )
...
def ADC_FullStartRing(index, config, calFile, Hz, buffer, depth):
    ...
    status = AIOUSB.ADC_FullStartRing(
        index, byref(configBuf), byref(configLen), calFile, byref(rateHz), buffer, depth
    )
...
def ADC_ReadData(index, config, scans, timeout):
    ...
    status = AIOUSB.ADC_ReadData(
        index, byref(configBuf), byref(scansToRead), byref(dataBuf), timeoutDuration
    )
    return status, dataBuf
...
def ADC_BulkContinuousCallbackStart(index, callback, context):
    ...
    return AIOUSB.ADC_BulkContinuousCallbackStart(
        index, c_uint32(1024 * 1024), c_uint32(64), c_uint32(context), Ccallback
    )
...
def displayBoardInfo(index):
    ...
    print(
        f"Board found at index {index} has deviceID {PID:x} = {name}, SN= {sn:x} [{dio} dio bytes]"
    )

def find_combinations_with_target_sum(arr, target):
    result = []
...
class Port:
    def __init__(self, name, types):
        self.output_bits = list("11111111")

        if types == "pull_down":
            self.bits = list("11111111")
        elif types == "pull_up":
            self.bits = list("00000000")

        self.name = name

    def return_output_bits(self):
        return int("".join(self.output_bits), 2)

    def return_hex(self):
        rev_bits = self.bits[::-1]
        res = "".join(rev_bits)
        print(res)
        return int(res, 2)
...
class Aiousb:
    def __init__(self):
        ...
        self.a = Port("attenuation_values", types="pull_down")
        self.b = Port("fett_and_switch", types="pull_up")

        status = DIO_Configure(
            diOnly,
            False,
            [0xFF, 0],
            [self.a.return_output_bits(), self.b.return_output_bits()]
        )
        if status == 0:
        ...
        self.main_switch_position = 0
        self.fet_positions = [1,2,3,4,5]
        self.attenutation_values = [1,2,4,8,16]

    def set_attenuation(self, atten):
        
        if atten == 0:
            for i, bit in enumerate(self.a.bits):
                self.a.bits[i] = "1"
        elif atten < 32:
            indxs = find_combinations_with_target_sum(self.attenutation_values, atten)
            if indxs[0]:
                for i, bit in enumerate(self.a.bits):
                    if i in indxs[0]:
                        self.a.bits[i] = "0"
                ...
                raise ValueError("THAT VALUE WONT WORK WITH CURRENT COMBONations of attenuation")
        else:
            raise ValueError(f"ATTENUATION NOT IN VALID RANGE {atten}")
        
        self.command()        

    def command(self):
        a_bits_set = self.a.return_hex()
        b_bits_set = self.b.return_hex()

        DIO_WriteAll(diOnly, [a_bits_set, b_bits_set])

        res = self.read_dio()

        a_bits_results = int(res[0])
        b_bits_results = int(res[1])

        if a_bits_set == a_bits_results and b_bits_set == b_bits_results:
            print("SUCKEESS")
        else:
            raise BufferError("FUCKED UP")
    ...
    def turn_on_fetts(self):
        for i, bit in enumerate(self.b.bits):
            if i in self.fet_positions:
                self.b.bits[i] = "1"

        self.command()

    def turn_on_main_switch(self):
        self.b.bits[self.main_switch_position] = "1"

        self.command()

    def turn_off_main_switch(self):
        self.b.bits[self.main_switch_position] = "0"

        self.command()

    def turn_off_fetts(self):
        for i, bit in enumerate(self.b.bits):
            if i in self.fet_positions:
                self.b.bits[i] = "0"
    ...
    def enable_rf(self):
        self.turn_on_fetts()
        time.sleep(.2)
        self.turn_on_main_switch()

    def disable_rf(self):
        self.turn_off_main_switch()
        time.sleep(.2)
        self.turn_off_fetts()