...
class Port:
    def __init__(self, name, types):
        ...
        self.name = name
        self.types = types
        self.main_switch_position = 4
        self.fet_positions = [0,1,2,3]
    ...
    def turn_on_main_switch(self):
        self.bits[self.main_switch_position] = "1"

    def set_attenuation(self, atten):
        if self.types == "attenuation_values":
            if not atten > 31 and not atten < 0:
                ...
                raise ValueError("ATTENUATION NOT IN VALID RANGE")
        else:
            raise TypeError("NOT CORRECT PORT TO SET ATTENUATION")
...
if __name__ == "__main__":  # main code section
    ...
    if True:  
        a = Port("A", "attenuation_values")
        b = Port("B", "fett_and_switch")

        status = DIO_Configure(
            diOnly,
            False,
            [0xFF, 0],
            [a.return_output_bits(), b.return_output_bits()]
        )


        status = DIO_WriteAll(diOnly, [a.return_hex(), b.return_hex()]) 
        print(status)
        ...
        a.set_attenuation(15)
        a_bits = a.return_hex()
        b.turn_on_fetts()
        b_bits = b.return_hex() 
        print(a_bits)
        print(b_bits)
        status = DIO_WriteAll(diOnly, [a_bits, b_bits])
