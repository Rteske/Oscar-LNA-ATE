ModularZT_NET45.dll  - .NET Class Library Object signed dll file build of Framework 4.5  - for programmers
...
The .NET DLL file include the following Functions:
...
22  int GetEthernet_PromptMN()

	return 1 if Prompt Model Name in TELNET is enabled.
	return 0 if Prompt Model Name in TELNET is disabled.	
	

23. int SaveEthernet_IPAddress(int b1,int b2,int b3,int b4)

	return 1 for success 
               0 for fail
	store the IPAddress (b1.b2.b3.b4)

24. int SaveEthernet_NetworkGateway(int b1,int b2,int b3,int b4)

	return 1 for success 
               0 for fail
	store the Gateway (b1.b2.b3.b4)

25. int SaveEthernet_SubnetMask(int b1,int b2,int b3,int b4)

	return 1 for success 
               0 for fail
	store the SubnetMask (b1.b2.b3.b4)

26. int SaveEthernet_PWD(String PWD)
	
	return 1 for success 
               0 for fail
	store the Password (PWD)

27. int SaveEthernet_TCPIPPort(int Port)

	return 1 for success 
               0 for fail
	store the TCP/IP Port (Port)

28.  int SaveEthernet_UseDHCP(int UseDHCP)
...
29. int SaveEthernet_UsePWD(int UsePWD)
...
30. int SaveEthernet_PromptMN(int Enable_PromptMN)

	return 1 for success 
               0 for fail
	Enable_PromptMN=1 to enable prompt model name in Telnet
    Enable_PromptMN=0 to disable prompt model name in Telnet
